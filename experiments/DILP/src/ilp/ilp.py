...
class ILP():
    ...
    def generate_ground_atoms(self):
        ...
        has_binary = any(pred.arity >= 2 for pred in p)
        unary_terms = [[Term(False, const)] for const in constants]
        binary_terms = []
        if has_binary:
            for const1 in constants:
                for const2 in constants:
                    ...
                    binary_terms.append([term1, term2])
        ...
        for pred in p:
            if pred.arity == 1:
                term_matrix = unary_terms
            elif pred.arity >= 2:
                term_matrix = binary_terms
            else:
                term_matrix = [[]]
            for term in term_matrix:
                atom = Atom([term[i] for i in range(0, pred.arity)], pred.predicate)
